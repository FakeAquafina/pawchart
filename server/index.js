...
app.put("/api/records/:id", (req, res) => {
  const { visit_date, clinic, vet, services, medications, total_cost, notes, reminder_date } = req.body;

  db.prepare(`
    UPDATE records SET visit_date=?, clinic=?, vet=?, services=?, medications=?,
      total_cost=?, notes=?, reminder_date=? WHERE id=?
  `).run(
    visit_date || null, clinic || null, vet || null,
    JSON.stringify(services || []), JSON.stringify(medications || []),
    total_cost || null, notes || null, reminder_date || null,
    req.params.id
  );

  // Sync the reminder row for this record
  db.prepare("DELETE FROM reminders WHERE record_id=?").run(req.params.id);
  if (reminder_date) {
    const record = db.prepare("SELECT * FROM records WHERE id=?").get(req.params.id);
    db.prepare(`INSERT INTO reminders (record_id, pet_id, pet_name, due_date, label) VALUES (?, ?, ?, ?, ?)`)
      .run(req.params.id, record.pet_id || null, record.pet_name || null, reminder_date, (services || [])[0] || "Follow-up");
  }
